...
fn empty_blockhashes_have_zero_l1_number_and_no_hashes() {
    ...
    assert_eq!(bh.l1_block_number(b).unwrap(), 0);
    assert!(bh.block_hash(b, 0).unwrap().is_none());
    assert!(bh.block_hash(b, 100).unwrap().is_none());
}
...
fn record_advances_l1_block_number() {
    ...
    let b = unsafe { &mut *state_ptr };
    bh.record_new_l1_block(b, 5, hash_n(5), ARBOS_V30).unwrap();
    assert_eq!(bh.l1_block_number(b).unwrap(), 6);
}
...
fn block_hash_returns_recorded_hash_for_recent_block() {
    ...
    let b = unsafe { &mut *state_ptr };
    bh.record_new_l1_block(b, 10, hash_n(10), ARBOS_V30)
        .unwrap();
    assert_eq!(bh.block_hash(b, 10).unwrap(), Some(hash_n(10)));
    assert!(bh.block_hash(b, 11).unwrap().is_none());
}
...
fn block_hash_outside_256_window_returns_none() {
    ...
    let b = unsafe { &mut *state_ptr };
    bh.record_new_l1_block(b, 500, hash_n(500), ARBOS_V30)
        .unwrap();
    assert!(bh.block_hash(b, 0).unwrap().is_none());
    assert!(bh.block_hash(b, 243).unwrap().is_none());
    assert_eq!(bh.block_hash(b, 500).unwrap(), Some(hash_n(500)));
}
...
fn record_new_block_fills_gaps_with_derived_hashes() {
    ...
    let b = unsafe { &mut *state_ptr };
    bh.record_new_l1_block(b, 0, hash_n(0), ARBOS_V30).unwrap();
    bh.record_new_l1_block(b, 5, hash_n(5), ARBOS_V30).unwrap();

    assert_eq!(bh.block_hash(b, 5).unwrap(), Some(hash_n(5)));
    let derived = bh
        .block_hash(b, 2)
        .unwrap()
        .expect("gap-filled hash present");
...
fn record_with_lower_number_is_noop() {
    ...
    let b = unsafe { &mut *state_ptr };
    bh.record_new_l1_block(b, 100, hash_n(100), ARBOS_V30)
        .unwrap();
    bh.record_new_l1_block(b, 50, hash_n(50), ARBOS_V30)
        .unwrap();
    assert_eq!(bh.l1_block_number(b).unwrap(), 101);
}
