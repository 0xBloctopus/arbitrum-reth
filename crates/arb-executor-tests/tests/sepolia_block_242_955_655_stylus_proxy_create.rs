...
use arb_executor_tests::helpers::{alice, alice_key, deploy_contract, fund_account, recover, sign_1559};
...
fn activate_stylus(
    ...
    state
        .programs
        .set_module_hash(codehash, activation.module_hash)
        .expect("set module hash");
    state
        .programs
        .set_program(codehash, program)
        .expect("set program");
}
...
fn run_scenario(activated_at_hours: u32) -> ScenarioOutcome {
    arb_stylus::pages::reset_stylus_pages();
    ...
    deploy_contract(harness.state(), STYLUS_IMPL, stylus_impl_code.clone(), U256::ZERO);
    deploy_contract(harness.state(), STYLUS_INIT, stylus_init_code.clone(), U256::ZERO);
